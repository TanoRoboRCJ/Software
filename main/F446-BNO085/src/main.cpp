...
/*
  ...
  Some special use cases may require turning on all or any special combo of
  sensor dynamic calibration.

  After the calibration settings are set, this example will output the
  x/y/z/accuracy of the mag and the i/j/k/real parts of the game rotation
  vector. https://en.wikipedia.org/wiki/Quaternions_and_spatial_rotation
  ...
  Originally written by Nathan Seidle @ SparkFun Electronics, December 28th,
  2017
...
void setup() {
    ...
    // For a final version of a project that does not need serial debug (or a
    // USB cable plugged in), Comment out this while loop, or it will prevent
    // the remaining code from running.
    ...
    // if (myIMU.begin() == false) {  // Setup without INT/RST control (Not
    // Recommended)
    ...
    // if (myIMU.setCalibrationConfig(SH2_CAL_ACCEL || SH2_CAL_GYRO ||
    //                                SH2_CAL_MAG) == true) {  // all three sensors
    //     // if (myIMU.setCalibrationConfig(SH2_CAL_ACCEL || SH2_CAL_MAG) == true)
    //     // { // Default settings if (myIMU.setCalibrationConfig(SH2_CAL_ACCEL)
    //     // == true) { // only accel
...
void loop() {
    ...
    if (myIMU.getSensorEvent() == true) {
        ...
        else if (myIMU.getSensorEventID() ==
